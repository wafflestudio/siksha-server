...
(function() {
  var app, combineCrawlingData, consignmentCrawling, crawlingJob, cronJob, directManagementCrawling, express, fs, getDataOfNextSunday, getTimeType, graduateCrawling, iconv, jqueryFile, jsdom, moment, nameConverter, promise, request, setPrice, vetCrawling, vetDataUpdatingJob, writeCrawlingData;
  ...
  vetCrawling = function(list, flag) {
        ...
        if (!error) {
          return jsdom.env({
            ...
            src: [jqueryFile],
              ...
              var $, dinner, dinnerTd, lunch, lunchTd, menus, tbody, todayIndex, tr;
              $ = window.jQuery;
              menus = [];
              todayIndex = new Date().getDay();
              if (!(todayIndex === 0 || (todayIndex === 6 && flag === "tomorrow"))) {
                tbody = $("table[bgcolor=dddddd] > tbody");
                tr = ($(tbody)).children().get((flag === "tomorrow" ? todayIndex + 1 : todayIndex));
          ...
          return jsdom.env({
            ...
            src: [jqueryFile],
              ...
              for (j = 0, len = restaurants.length; j < len; j++) {
                ...
                list.push({
                  restaurant: nameConverter.nameMap.get(restaurant),
                  menus: menus
                });
              }
              return resolve(list);
  ...
  combineCrawlingData = function(flag, callback) {
    var consignments, directManagements, graduates, result, vets;
    result = [];
    directManagements = [];
    consignments = [];
    graduates = [];
    vets = [];
    return promise.all([directManagementCrawling(directManagements, flag), consignmentCrawling(consignments, flag), graduateCrawling(graduates, flag), vetCrawling(vets, flag)]).then(function() {
      var j, k, l, len, len1, len2, len3, m, restaurant;
      ...
      for (m = 0, len3 = vets.length; m < len3; m++) {
        restaurant = vets[m];
        result.push(restaurant);
      }
